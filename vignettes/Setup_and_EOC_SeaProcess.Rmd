...
6.  Once you have created your datasheet you need to make sure the
    ...
    -   Next scroll down to the "Create Datasheets" portion of the
        ...
        -   SeaProcess will try to create an ODV formatted output for
            any addition datasheets created. If this causes an issue,
            set `odv_export = FALSE` withing `create_datasheet()`
...
"summary_input" sheet. For example, if you have a datasheet with blank
columns "temp," "sal," chla," etc and additional columns. This means we
can have a single datasheet for observational data or surface station
data instead of separate summary sheets for each. To do this for an obs
sheet: - Make a copy of the "summary_input.xls", and rename it
"<cruiseID>\_obs_input.xls." - Keep the columns already present and add
the additional columns you want. - In the `process_data.R` script first
...
create quick maps of station locations.
