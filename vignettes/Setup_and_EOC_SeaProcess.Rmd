...
If you need to add variables to a datasheet for a specific project, just add a new column. 
...
-   Once you have created your datasheet you need to make sure the
    ...
    -   Next scroll down to the "Create Datasheets" portion of the
        ...
        -   Seaprocess will automatically do some formatting to try and standardize column names to be easier to work with in R. To skip this step in datasheet processing, set `preserve_col_names = T` within `create_datasheet()`. Because of the additional processing that happens with the autocreated datasheets, this argument will only work with custom datasheets. 
...
Calculation sheets can be setup to autofill calculated values from any SEA calc sheet into a datasheet with a matching column name and a 'bottle' column. We will most often use it to enter water chemistry values (nutrients, ph, chla) into the bottle datasheet for hydrocasts or niskins on the wire.
...
Alternatively, say you want to setup calculation sheets for reef data. Your function could look like:
`create_datasheet(reef_input, data_type = "REEF", cruiseID = cruiseID, process_calc = T, calc_folder = calc_folder)` **NOTE:** Each deployment datasheet can look through one folder of calc sheets. If you are running calculation sheets for hydrocasts AND for reef data, you will need to define two separate calc_folders. 
...
Within the `station_plots` folder are three scripts (as of 1/23/2025) to create quick maps of station locations. 

## Setup

To setup the plot scripts, follow the steps at the beginning to define your cruiseID and input and output file paths. The one additional setting that may need to be changed is the padding on the latitude and longitude of the plots. To do that scroll down to a line that looks like: `plot_limits(data, lat_pad = 3, lon_pad = 5)` and change the value of `lat_pad` and `lon_pad`. I recommend running the "NoBathymetry.R" script first. Without needing to load in bathymetry data, it goes much faster. 

## Running

Once the script is setup, you can run the whole script with "Source" or "ctrl + alt + r", or run the script line by line. The plots will be created for each datasheet in the csv folder and will output to the "output/plots" folder.

If you are the type to try an automate things, you can add a line `source("plot_scripts/StationPlotsNoBathymetery.R")` at the end of your `process_data.R` script. When you run `process_data.R` it will also run the entirety of that station plots script without having to navigate to it and run it separately. 

## Editing

Feel free to edit the plot scripts to fit your cruise's needs! Just be sure the science team is on the same page about changes and you save a clean copy somewhere in case you accidentally make a change you can't revert.
...
-   Each function within the package has lots of information to
    ...
    put a `?` in front of it. For example run: `?seaprocess::create_summary()`.
